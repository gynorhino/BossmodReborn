...
sealed class OctupleSwipe(BossModule module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.OctupleSwipe1)
        {
            aoes.Add(new(new AOEShapeCone(40f, 45f.Degrees()), caster.Position, Module.PrimaryActor.Rotation + caster.Rotation, WorldState.FutureTime(10d), risky: false));
        }
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (aoes.Count != 0)
        {
            ...
            {
                case (uint)AID.OctupleSwipe2:
                case (uint)AID.OctupleSwipe3:
                case (uint)AID.OctupleSwipe4:
                    aoes.RemoveAt(0);
                    break;
            }
        }
